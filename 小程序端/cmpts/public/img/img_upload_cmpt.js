...
Component({
	...
	properties: {
		...
		upDirectDir: { //直接上传的路径，不为空：则在非测试模式下直接上传
			type: String,
			value: '',
		},
	},
			...
			wx.chooseMedia({
				...
				success: async (res) => {
					...
					for (let k = 0; k < res.tempFiles.length; k++) {
						...
						if (!setting.IS_DEMO) { //for smartApp
							// 非测试模式直接上传
							path = await cloudHelper.transTempPicOne(path, this.data.upDirectDir, this.data.isCheck);
						}
